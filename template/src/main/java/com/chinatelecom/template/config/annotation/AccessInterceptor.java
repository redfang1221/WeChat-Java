...
//    }
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if (handler instanceof HandlerMethod) { //Action 验证
            HandlerMethod handlerMethod = (HandlerMethod) handler;
            Access access = handlerMethod.getMethodAnnotation(Access.class);
            if (access != null) { //不做任何拦截
                if (access.login() && accessValidator != null) {
                    if(request.getRequestURI().equals("/") && !accessValidator.loginValidator(request)){
                        //直接重定向到登录页面
                        response.sendRedirect(request.getContextPath()+"/login");
                        return false;
                    }
                    //验证登录
                    if (!accessValidator.loginValidator(request)) {
                        //验证登录失败
                        //判断是否是ajax访问
